...
class MathChallengeGame
{
    ...
    static void AskQuestion()
    {
        ...
        try
        {
            ...
            if (Math.Abs(playerAnswer - correctAnswer) < 0.0001)
            {
                score += 10;
        ...
        try
        {
            using (StreamReader reader = new StreamReader("math_game_save.txt"))
            {
                ...
                while ((line = reader.ReadLine()) != null)
                {
                    ...
                    gameData[round, question] = new Question(parts[2], double.Parse(parts[3]), double.Parse(parts[4]));
                }
            }
...
class Question
{
    ...
    public double PlayerAnswer { get; }
    public double CorrectAnswer { get; }
    public bool IsCorrect => Math.Abs(PlayerAnswer - CorrectAnswer) < 0.0001;

    public Question(string questionText, double playerAnswer, double correctAnswer)
    {
